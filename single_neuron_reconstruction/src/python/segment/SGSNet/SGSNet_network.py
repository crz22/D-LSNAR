    ...
    def __init__(self, features, WH, M, G, r, stride=1, L=8):
        ...
        for i in range(M):
            self.convs.append(nn.Sequential(
                nn.Conv3d(features, features, kernel_size=(5 + i * 2, 5 + i * 2, 3), stride=stride,
                          padding=(2 + i, 2 + i, 1), groups=G),  # kernel_size=3+i*2   padding=1+i
...
class SGSNet(nn.Module):
    def __init__(self, in_dim=1, out_dim=2, num_filters=32):
